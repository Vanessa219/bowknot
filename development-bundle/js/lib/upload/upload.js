    ...
    $.extend($.fn,{
        ...
        upload: function(options) {
            if (arguments.length === 1 && typeof(options) === "string") {
                var method = options.charAt(0).toUpperCase() + options.substr(1, options.length - 1);
                return $.fn['uploadify' + method ].apply(this);
            }
            $(this).each(function(){
                var settings = $.extend({
                    id              : $(this).attr('id'), // The ID of the object being Uploadified
                    uploader        : 'upload.swf', // The path to the uploadify swf file
                    script          : 'uploadOrRemoveFile?operate=flashUpload', // The path to the uploadify backend upload script
                    ...
                    if (event.data.action(event, ID, fileObj) !== false) {
                        ...
                        $(queue).append('<div id="' + $(this).attr('id') + ID + '" class="uploadifyQueueItem">\
                                            ...
                                            <div class="upload-middle">\
                                                <a class="cancel" href="javascript:jQuery(\'#' + jQuery(this).attr('id') + '\').uploadifyCancel(\'' + ID + '\')"></a>\
                                                <span class="icon"></span><span class="fileName">' + fileName + ' (' + byteSize + suffix + ')</span><span class="percentage"></span>\
                    ...
                    if (event.data.action(event, ID, fileObj, data, clearFast) !== false) {
                        if (remove) {
                            ...
                            $("#" + $(this).attr('id') + ID).fadeOut(fadeSpeed, function() {
                                $(this).removeData("response").remove();
                            });
                        }
                    }
                });
                $(this).bind("uploadifyClearQueue", {
                ...
                $(this).bind("uploadifyProgress", {
                    ...
                    if (event.data.action(event, ID, fileObj, data) !== false) {
                        $("#" + $(this).attr('id') + ID + "ProgressBar").animate({
                            'width': data.percentage + '%'
                        },250,function() {
                            if (data.percentage == 100) {
                                $(this).closest('.uploadifyProgress').fadeOut(250,function() {
                                    $(this).remove()
                                });
                            }
                        });
                ...
                $(this).bind("uploadifyComplete", {
                    ...
                    var responseArray = eval(unescape(response));
                    if (event.data.action(event, ID, fileObj, responseArray, data) !== false) {
                        $("#" + $(this).attr('id') + ID).find('.percentage').text(' - 上传完成');
                        if (settings.removeCompleted) {
                            $("#" + $(event.target).attr('id') + ID).fadeOut(250,function() {
                                $(this).remove()
                            });
                        }
                        $("#" + $(event.target).attr('id') + ID).addClass('completed').data("response", responseArray);
                    }
                });
